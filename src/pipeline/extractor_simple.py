"""
Simplified PDF Extraction Module for Threat Intelligence Pipeline
Lightweight version without heavy OpenCV dependencies
"""
...
class PDFExtractor:
    """Simplified PDF extraction without OpenCV"""
    ...
    def _extract_page(self, page, page_num: int, pdf_path: str) -> PDFPage:
        ...
        if not text or len(text.strip()) < 50:
            if self.enable_ocr:
                ...
                text = self._ocr_page_simple(page, page_num)
        ...
        if self.extract_images:
            images = self._extract_images_simple(page, page_num)
    ...
    def _extract_images_simple(self, page, page_num: int) -> List[Dict]:
        """Simplified image extraction without OpenCV"""
        images = []
    ...
    def _ocr_page_simple(self, page, page_num: int) -> str:
        """Simplified OCR without OpenCV preprocessing"""
        try:
            # Convert page to image using pdfplumber
            im = page.to_image(resolution=200)
            
            # Save temporary image
            temp_image_path = self.temp_dir / f"page_{page_num}.png"
